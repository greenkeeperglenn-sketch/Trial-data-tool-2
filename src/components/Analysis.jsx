...
const Analysis = ({ config, gridLayout, assessmentDates, selectedAssessmentType }) => {
  ...
  const [groupBy, setGroupBy] = useState('treatment'); // 'treatment' or 'date'
        ...
        {chartType === 'boxplot' && (
          <div className="overflow-x-auto">
            {(() => {
              ...
              if (allValues.length === 0) return <div className="text-gray-500">No data available</div>;
              ...
              if (groupBy === 'treatment') {
                // Group by treatment: each treatment shows all dates
                return (
                  <>
                        ...
                        <div key={treatmentIdx} className="flex flex-col items-center">
                          ...
                          <div className="flex gap-2 items-end h-64">
                            {assessmentDates.map((dateObj, dateIdx) => {
                              ...
                              const colorClass = dateColors[dateIdx % dateColors.length];

                              return (
                                <div key={dateIdx} className="flex flex-col items-center">
                                  <div className="relative h-48 w-12 bg-gray-100 rounded border">
                                    <div className={`absolute w-full ${colorClass} border-2`} style={{ bottom: `${scale(q1)}px`, height: `${Math.max(2, scale(q3) - scale(q1))}px` }} />
                                    <div className="absolute w-full h-1 bg-black" style={{ bottom: `${scale(median)}px` }} />
                                    <div className="absolute left-1/2 w-0.5 bg-black -translate-x-1/2" style={{ bottom: `${scale(min)}px`, height: `${Math.max(0, scale(q1) - scale(min))}px` }} />
                                    <div className="absolute left-1/2 w-0.5 bg-black -translate-x-1/2" style={{ bottom: `${scale(q3)}px`, height: `${Math.max(0, scale(max) - scale(q3))}px` }} />
                                  </div>
                ...
                // Group by date: each date shows all treatments
                return (
                  <>
                    <div className="flex gap-8 items-end min-w-max pb-8">
                      {assessmentDates.map((dateObj, dateIdx) => {
                              ...
                              {config.treatments.map((treatment, treatmentIdx) => {
                                ...
                                return (
                                  <div key={treatmentIdx} className="flex flex-col items-center">
                                    ...
                                    <div className="text-xs mt-2 text-center truncate w-12" title={treatment}>{treatment.substring(0, 6)}</div>
                                    <div className="text-xs text-gray-600">{treatmentStat.group}</div>
                                  </div>
                                );
                              })}
                    ...
                    <div className="mt-4 text-xs text-gray-600">
                      <p>Each date shows box plots for all treatments side by side</p>
                      <p>Letters below indicate statistical groupings (Fisher's protected LSD test)</p>
                    </div>
                  </>
                );
        ...
        {chartType === 'bar' && (
          <div className="overflow-x-auto">
            {(() => {
              ...
              if (groupBy === 'date') {
                // Group by date: each date shows all treatments (current view)
                return (
                  <>
                    <div className="space-y-6">
                      {assessmentDates.map((dateObj, dateIdx) => {
                            ...
                            <div className="flex gap-4 items-end h-56">
                              {stats.treatmentStats.map((ts, idx) => (
                                <div key={idx} className="flex flex-col items-center flex-1 max-w-24">
                                  <div className="text-xs font-medium text-gray-600 mb-1">{ts.mean.toFixed(1)}</div>
                                  <div className="text-xs text-blue-600 font-bold mb-1">({ts.group})</div>
                                  <div className="w-full rounded-t" style={{ height: `${scale(ts.mean)}px`, backgroundColor: treatmentColors[idx % treatmentColors.length] }} />
                                  <div className="text-xs mt-2 text-center truncate w-full" title={config.treatments[ts.treatment]}>{config.treatments[ts.treatment]}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                    ...
                    <div className="mt-4 text-xs text-gray-600">
                      <p>Each date shows bar chart of all treatments</p>
                ...
                return (
                  <>
                    <div className="space-y-6">
                      {config.treatments.map((treatment, treatmentIdx) => (
                        <div key={treatmentIdx} className="border-b pb-4">
                          <div className="font-medium mb-3">{treatment}</div>
                          <div className="flex gap-4 items-end h-56">
                            {assessmentDates.map((dateObj, dateIdx) => {
                              const stats = calculateStats(dateObj);
                              if (!stats) return null;
                              ...
                              return (
                                <div key={dateIdx} className="flex flex-col items-center flex-1 max-w-24">
                                  <div className="text-xs font-medium text-gray-600 mb-1">{ts.mean.toFixed(1)}</div>
                                  <div className="text-xs text-blue-600 font-bold mb-1">({ts.group})</div>
                                  <div className="w-full rounded-t" style={{ height: `${scale(ts.mean)}px`, backgroundColor: dateColors[dateIdx % dateColors.length] }} />
                                  <div className="text-xs mt-2 text-center truncate w-full" title={dateObj.date}>{dateObj.date}</div>
                                </div>
                              );
                            })}
                          </div>
                    ...
                    <div className="mt-4 text-xs text-gray-600">
                      <p>Each treatment shows bar chart across all dates</p>
                      <p>Letters indicate statistical groupings (Fisher's protected LSD test)</p>
                    </div>
                  </>
                );
                    ...
                    <div className="flex flex-wrap gap-4 mt-4 justify-center">
                      {config.treatments.map((treatment, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <div className="w-4 h-4 rounded" style={{ backgroundColor: treatmentColors[idx % treatmentColors.length] }} />
                          <span className="text-sm">{treatment}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-4 text-xs text-gray-600">
                      <p>Each line shows one treatment across all dates</p>
                      <p>X-axis: Assessment dates, Y-axis: Treatment means</p>
                    </div>
                  </div>
                );
              } else {
